...
function [lbpxy,lbpxt,lbpyt,Histogram] = LBPTOP(VolData, FxRadius, FyRadius, TInterval, NeighborPoints, TimeLength, BorderLength, bBilinearInterpolation, Bincount, Code)
...
topxc = width - BorderLength - (BorderLength + 1) + 1;
topyc = height - BorderLength - (BorderLength + 1) + 1;
toptt = Length - TimeLength - (TimeLength + 1) + 1;

lbpxy = zeros(topyc,topxc,toptt);
lbpxt = zeros(topyc,topxc,toptt);
lbpyt = zeros(topyc,topxc,toptt);
ti=0;
tx=0;
ty=0;
    ...
    for i = TimeLength + 1 : Length - TimeLength
        ...
        for yc = BorderLength + 1 : height - BorderLength
            ...
            for xc = BorderLength + 1 : width - BorderLength
                ...
                %%LBP and uniform LBP
                if Bincount == 0
%                     minLBP = RotLBP(BasicLBP, NeighborPoints);
%                     lbpxy(ty, tx, ti) = RotLBP(BasicLBP, NeighborPoints(1));
                    lbpxy(ty, tx, ti) = BasicLBP;
...
% rilbptopcell =[lbpxy,lbpxt,lbpyt];
% 
% 
% %% normalization
for j = 1 : 3
